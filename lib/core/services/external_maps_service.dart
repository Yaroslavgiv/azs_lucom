...
class ExternalMapsService {
  ...
  static Uri yandexRouteUri(double lat, double lon) => Uri.parse(
        'https://yandex.ru/maps/?rtext=~$lat,$lon&rtt=auto',
      );

  static Uri yandexPointUri(double lat, double lon) => Uri.parse(
        'https://yandex.ru/maps/?pt=$lon,$lat&z=16&l=map',
      );

  /// Маршрут до точки (| — откуда «моё местоположение» в 2ГИС).
  static Uri twoGisRouteUri(double lat, double lon) => Uri.parse(
        'https://2gis.ru/directions/points/|$lon,$lat',
      );

  static Uri twoGisPointUri(double lat, double lon) => Uri.parse(
